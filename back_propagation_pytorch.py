...
def go_forward(x_inp, w1, w2):
    """
    Функция прохода по НС.

    Parameters:
        x_inp (torch.FloatTensor): входные данные
        w1 (torch.FloatTensor): матрица весов для первого слоя
        w2 (torch.FloatTensor): матрица весов для второго слоя

    Returns:
        y (torch.FloatTensor): выходное значение НС
        z1 (torch.FloatTensor): сумма на входах нейронов первого слоя
        z2 (torch.FloatTensor): сумма на входе нейрона второго слоя
    """
    # вычисляем сумму на входах нейронов первого слоя
    z1 = torch.mv(w1[:, :3], x_inp) + w1[:, 3]

    # вычисляем значения на выходах нейронов первого слоя
    s = act(z1)

    # вычисляем сумму на входе нейрона второго слоя
    z2 = torch.dot(w2[:2], s) + w2[2]

    # вычисляем выходное значение НС
    y = act(z2)
    return y, z1, z2
